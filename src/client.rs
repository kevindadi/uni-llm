...
impl Client {
    ...
    pub async fn generate(&self, request: GenerationRequest) -> Result<GenerationResponse, DashScopeError> {
        let path = if is_multimodal_model(&request.model) {
        ...
        let body_text = resp.text().await.map_err(DashScopeError::RequestBuildError)?;
        ...
        if is_error {
            let message = raw
                .message
                .filter(|m| !m.is_empty())
                .unwrap_or_else(|| format!("API 返回异常。原始响应: {}", &body_text[..body_text.len().min(500)]));
